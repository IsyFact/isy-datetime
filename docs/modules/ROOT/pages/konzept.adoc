...
Diese Seite beschreibt die konzeptionellen Grundlagen der Verarbeitung von Datums- und Zeitwerten in der IsyFact.
Sie definiert Vorgaben für die einheitliche Verwendung von Datums- und Zeitfunktionen sowie die Behandlung von
Datums- und Zeitwerten in Anwendungen und bildet so die Grundlage einer technisch und fachlich einheitlichen Umsetzung.

Die aus diesem Konzept abgeleitete Umsetzung der Verarbeitung von Datums- und Zeitwerten im Rahmen der
Anwendungsentwicklung ist in den xref:nutzungsvorgaben.adoc[] beschrieben.
...
<<table-InpFormZRaum,Die folgende Tabelle>> zeigt Beispiele für die Eingabewerte von Zeiträumen.
...
Die Bibliothek muss sogenannte _ungewisse_ Datumswerte verarbeiten können.
Dabei sind Teile eines Datumswerts zunächst ungewiss.
...
Es gibt eine Einschränkung bei der Konkretisierung ungewisser Datumswerte.
Da die Ungewissheit durch Zeiträume abgebildet wird, müssen ungewisse Datumswerte immer einen in sich abgeschlossenen Zeitraum bilden.
Nicht abbildbar ist z.B. "Geburtsdatum: der 24. eines beliebigen Monats des Jahres 1976", da dies nicht als Zeitraum darstellbar ist.
...
Falls einige der Anforderungen aus dem Kapitel <<anforderungen>> komplett durch die Funktionalität dieser API abgedeckt sind, wird in den xref:nutzungsvorgaben.adoc[Nutzungsvorgaben] die entsprechende Verwendung der API mit Verweis auf die Standarddokumentation knapp erläutert.
...
* Definition der Transportobjekte zur Übermittlung ungewisser Datumswerte über Schnittstellen
...
Die Anzeige und Eingabe dieser unbekannten Werte erfolgt hierbei über Nullen, wie die <<table-OldUmunZeitw>> zeigt.
...
Eine Übersicht dieses Vorgangs zeigt die <<table-UmsSonderNull>>.
...
Des Weiteren muss die Bibliothek die Eingabe unbekannter Datumsbestandteile mittels "x" unterstützen.
Diese Möglichkeit wird die Eingabe unbekannter Datumsbestandteile mittels Nullen mittelfristig ablösen und unterstützt Eingaben im ISO-8601 Datumsformat.
Eine Übersicht der so umgesetzten Spezialfälle zeigt die <<table-UmsSonderX>>.
...
nicht gesetzt (null)
|Tag unbekannt (ISO-8601) |1966-05-xx |Zeitraum: +
1.5.1966 – 31.5.1966
|Tag und Monat unbekannt (ISO-8601) |1966-xx-xx |Zeitraum: +
1.1.1966 – 31.12.1966
|Datum komplett unbekannt (ISO-8601) |xxxx-xx-xx |Zeitraum: +
nicht gesetzt (null)
|====

[[testunterstuetzung-1]]
=== Testunterstützung
